        ...
        <%@ page import="util.DatabaseConnection" %>
            <%@ page session="true" %>
                ...
                <% Employee emp=(Employee) session.getAttribute("employee"); if (emp==null) {
                    response.sendRedirect("Login.jsp"); return; } String employeeId=emp.getEmpId(); Connection
                                ...
                                <table class="claim-table">
                                    ...
                                    <tbody>
                                        <% try { conn=DatabaseConnection.getConnection(); String
                                            sql="SELECT SUBMISSIONID, CLAIMTYPE, SUBMISSIONDATE, STATUS "
                                            + "FROM SUBMISSION " + "WHERE EMPLOYEEID = ? "
                                            + "ORDER BY SUBMISSIONDATE DESC" ; ps=conn.prepareStatement(sql);
                                            ps.setString(1, employeeId); rs=ps.executeQuery(); boolean hasData=false;
                                            while (rs.next()) { hasData=true; int id=rs.getInt("SUBMISSIONID"); String
                                            type=rs.getString("CLAIMTYPE"); String status=rs.getString("STATUS"); %>
                                            <tr>
                                                <td>
                                                    <%= type %>
                                                </td>
                                                ...
                                                <td>
                                                    ...
                                                    <a href="ViewClaimDetail.jsp?id=<%= id %>" class="btn blue">
                                                        View Claim
                                                    </a>

                                                    <!-- UPDATE CLAIM (Draft only) -->
                                                    <% if ("Draft".equalsIgnoreCase(status)) { %>
                                                        <a href="UpdateClaim1.jsp?id=<%= id %>" class="btn blue">
                                                            Update Claim
                                                        </a>
                                                        <% } else { %>
                                                            ...
                                                            <% } %>
                                                                ...
                                                                <% if ("Draft".equalsIgnoreCase(status)) { %>
                                                                    ...
                                                                    <form action="SubmitDraftServlet" method="post"
